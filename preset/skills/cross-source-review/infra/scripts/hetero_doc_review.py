...
malformation, providers}`. The CSR-I4 convergence driver (built separately) calls this
wrapper once per round and assembles the convergence-record itself.
...
Flags used:
  claude -p
    ...
    --max-budget-usd <cap>             hard cost breaker per subprocess (a full review
                                       of a large doc can cost >$1 — set headroom)
...
def _claude_argv(
    ...
    Preserved verbatim from pd's hetero_review.py (same flag surface)."""
    argv = [
        "claude",
        "-p",
        "--settings",
...
def _run_claude_once(
    ...
    Signature + DEGRADE logic preserved EXACTLY from pd's hetero_review.py (ADR #41).
    """
    base = {"findings": None, "hook_count": 0, "error_subtype": None, "errors": []}
    if dry_run:
        if dry_malform:
...
def main():
    ...
    ap.add_argument(
        "--profile",
        default=os.environ.get("HETERO_DOC_PROFILE", ""),
    ...
    ap.add_argument(
        ...
        type=float,
        default=4.0,
        help="Hard cost breaker per subprocess. Default 4.0 leaves headroom under a "
        "typical 5.0 global cost cap — CC total_cost_usd over-reports vs DeepSeek "
        "cache-aware billing (ADR #40 (h)(i)), and a cold multi-tool doc review can "
        "cost >$1. If a review still trips the cap it DEGRADES (ADR #41), not rewrites.",
    )
    ap.add_argument(
        "--allowed-tools",
