...
| `/solidforge:arm-tools` command | arm-tools command (same procedure, same `arm.py`) | Layer-2 project provisioning unchanged; targets `AGENTS.md`. |
...
| Platform-imposed decoupling boundary (§4.4) | Claude Code's process-level provider binding forces heterogeneity out of process | The DSH in-process subagent registry is single-provider (DeepSeek) the same way — the heterogeneous ring stays an OS subprocess (`claude -p` / other runtimes). The port's `model-routing.md` documents the inversion of the paper's worked example. |
