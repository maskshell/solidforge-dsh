...
> `QWEN_TOKEN_PLAN_CN_ANTHROPIC_AUTH_TOKEN`) so one env-file FORMAT
...
shared env-file FORMAT; `<ROUTE>_API_KEY` is the user-authored-profile
...
var from the shared three-harness var convention; the route-derived
