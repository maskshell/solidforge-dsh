...
precise sense --- an orthogonality claim that rests on the
self-certification conjecture of §3.1, which we stipulate rather than
establish}: it tacitly assumes the verification source \emph{is} the
...
agent-reliability literature --- a synthesis of that body of work rather
than an adoption from one named source, though the same triad is
independently enumerated as the recurring challenges of long-horizon
agent workflows in LongHorizon-Harness (Ma et al.~2026), which also
observes that task execution and completion assessment remain coupled in
current harnesses and that auditors need evidence ``obtained directly
from the environment rather than by the executor's completion claim''
--- naming the coupling problem our Axis B addresses, without the axis
framing. The orthogonality claim below is relative to this three-mode
picture, not absolute:
...
mocking away a real dependency, over-fitting existing test inputs,
satisfying a local interface contract while leaving the upstream root
cause unaddressed, or removing public API surface that has no in-repo
callers while external consumers still import it (documented at
production scale in Teknium 2026; §7).
...
LLM-as-judge bias literature, which has measured same-family evaluators
scoring their own outputs higher while human annotators rate them equal
(Wataoka et al.~2024; Panickssery et al.~2024; §7): a verifier
co-trained with the verified cannot detect errors in the region where
both are blind. This conjecture is load-bearing for the necessity
framing, and the analysis in this paper assumes its strong form --- a
co-trained verifier shares the verified's blind spots wholesale ---
under which same-source review cannot reach latent specification gaming.
We flag, rather than hide, that this is stipulative: under a weaker
reading (same-family reviewers detect \emph{some} shared blind spots),
Axis B shifts from logically necessary to strongly advisable, though the
two-axis \emph{structure} survives, because partial detection still
leaves an undetected residue that only a differently-grounded oracle can
reach. The absolute reachability claims in this paper are to be read
under this stipulated strong form. The agent cannot write a test that
probes a blind spot it has; a same-family reviewer cannot flag a proxy
gap it also fails to see. The same channel that produces the code
produces its validation, and the failure class we care about is, at its
core, the latent one --- the shared-blind-spot region no same-source
channel can probe (its salient regime --- auditable tampering --- is
separable; §4.3).
...
\textbf{Criterion.} An oracle is heterogeneously decoupled $\Longleftrightarrow$ its
...
expose. \#\#\# 6.5 Retrospective evidence from production runs

The framework above predicts two observable signatures in a production
convergence loop that runs both a same-source and a
heterogeneously-decoupled review leg: (i) the two legs' discoveries
should substantially diverge (each oracle's blind-spot set differs,
§4.4); (ii) a fresh-context reviewer should catch defects that the
loop's own earlier fixes introduce --- the maker-checker asymmetry of
§3.2(iii). We test neither causally; we report what the production
record corpus shows, with its limitations stated.

\textbf{Corpus.} 46 convergence-run units carrying 797 findings from
their authoritative aggregate records (the naive per-file count at first
harvest was 1,461 --- the difference is the corpus's documented
round-file/aggregate duplication, resolved by the unit rules), from the
reference implementation's own doc-convergence runs and five consumer
codebases, harvested read-only from durable run records (one
author-maintained evidence corpus; consumer projects pseudonymized).
Findings are UNADJUDICATED reviewer claims --- counts are claim-counts,
not confirmed-defect counts; severity strata are reported separately and
honest-disclosure items are excluded from defect denominators.

\textbf{Signature (i) --- leg divergence.} Among the 359 defect-severity
findings whose producing leg is attributable from surviving per-leg
records, the same-source leg alone discovered 192 (53.5\%) and the
heterogeneous leg alone 167 (46.5\%). Within this attribution coverage
(54\% of defect findings; the remainder sit in units without per-leg
records), each leg alone would have lost roughly half of what the pair
recovered. This is the retrospective analogue of the divergence
observation in Open Problem 8 --- not the known-gap test itself: without
ground truth on which findings are true defects, divergence here
measures discovery complementarity, not verdict correctness. Both-report
duplication cannot be measured at id level (cross-leg identifiers never
coincide); a kind-and-location pair adjudication would move findings
from single-leg buckets into the both-bucket and cannot remove them, so
the single-leg loss estimate is an upper bound.

\textbf{Signature (ii) --- fix-introduced defects.} An LLM-assisted
coding pass over fix-vocabulary-filtered candidate findings (319 of 667
multi-round defect findings), spot-checked against record full text with
zero disagreement (5/5 positives, 3 negatives drawn, 2 effective after
one unverifiable lookup-miss (docs/coding-pass-verification-protocol.md
rule 1); single-seat limitation disclosed), with a second independent
seat re-adjudicating a 20\% subsample at 96.9\% strict-match agreement
(zero genuinely disputed positives), classifies 23 findings (3.4\% of
all multi-round defect findings; 7.2\% of the enriched candidate pool)
as defects explicitly attributed by the finding itself to an EARLIER FIX
in the same run --- residues of incomplete class-fixes, overshoots, and
rewording regressions. The pattern is pervasive across runs and engines
(positives recorded in five of the ten units the candidate pool spans)
rather than an artifact of any single integration. Every such finding
was caught within the loop, by a reviewer context that did not author
the fix under review. The causal reading --- ``caught only because the
reviewer was fresh'' --- is a counterfactual this corpus cannot carry
and we do not claim it.

\textbf{Limitations.} No control baseline exists (nothing here compares
the loop against single-pass review); environment provenance is
recoverable for a minority of units, so results are not stratified by
model era; the corpus is a survivorship-biased snapshot (degraded and
unrecorded runs are underrepresented); the coding pass is single-seat
with a consistency re-run reported alongside. A per-leg attribution pass
(multi-method: leg-file membership, cross-leg pair adjudication,
identifier-naming convention, peer-overlap) resolved 9 of the 12
originally unattributed fix-introduced positives, yielding a same-source
/ heterogeneous split of 17 / 3 --- a skew \emph{toward} the same-family
reviewer, not away. The heterogeneous leg's unique contribution to this
corpus is discovery complementarity (the 44.5\% of attributable findings
only it found), not fix-residue detection --- a genuinely unexpected
asymmetry worth noting as a limitation on any strong reading of the
heterogeneous oracle's role. These bound the section's claim to: in this
production corpus, the two oracle legs' discoveries are near-symmetric
complements, and fix-introduced defects are a recurring, catchable class
--- consistent with, not proof of, the two-axis frame's predictions.
...
another LLM is reliable or biased. The bias our conjecture rests on is
now measured, not merely suspected: GPT-4 exhibits significant
self-preference as a judge, with the bias tracking perplexity-based
familiarity (Wataoka et al.~2024), and LLM evaluators recognize and
favor their own generations --- scoring them higher even where human
annotators judge the texts equal in quality, with self-recognition
capability correlating linearly with self-preference strength
(Panickssery et al.~2024). At the detection frontier in code
environments, the TRACE benchmark (Deshpande et al.~2026) reports a
frontier detector (GPT-5.2, high reasoning) catching only 63\% of
injected reward hacks, with the studied detectors showing high mutual
agreement (Cohen's $\kappa$ = 0.812) --- consistent with our §3.1 reading:
detector families share blind spots, so agreement among same-tier
detectors is precisely not coverage; coverage comes from family
diversity, not detector count. Wahi (2026) names the Goodhart dynamic in
operational form for self-improvement loops --- the optimizer converges
to the decision boundary of whatever grades it, not to the boundary of
the task --- for evaluator loops generally; our contribution is the
coding-loop-specific framing (proxy specification gaming via test
manipulation), the blind-spot-set criterion (§4.1), and the
Process/Outcome split as a schema-level mitigation, not the discovery of
same-source bias. The Evaluator Stress Test (EST; Shihab et al.~2025)
detects proxy gaming in RL/LLM-alignment by separating
exploitable-sensitivity from content-driven improvement via controlled
perturbations --- a score-decomposition axis distinct from our
integrity/adequacy (salient/latent) bifurcation (§4.3); its
...
\textbf{Cheating detection, process supervision, and ensembles.} Three
adjacent lines stop short of the axis claim. Capped evaluation with
randomized tests (Lodkaew et al.~2026) explicitly separates cheating
from unintended failure modes --- the closest prior move to ours, but
made at the level of failure \emph{phenomenon} for benchmark validity
and statistical detection, not at the level of \emph{defense mechanism}
(§3.3): which oracle tiers can defend which failure classes is not its
question. Process supervision (Lightman et al.~2023) shows step-level
verification catching reasoning errors that final-answer checks miss;
the vocabulary overlaps our Process/Outcome split (§4.2) accidentally
--- their process/outcome distinguishes \emph{training reward signals},
ours separates \emph{loop convergence from correctness verdicts}, and
their result (process signals catch what outcome checks miss) is
orthogonal to our claim (process convergence cannot certify outcome
correctness at all). Ensemble work for code generation and repair
(Vallecillos-Ruiz et al.~2025) quantifies both faces our retrospective
corpus shows (§6.5): diverse ensembles realize most of the theoretical
complementarity potential --- individual models solve unique bugs no
other member can --- while naive configurations oscillate, one model's
fix flagged by the next. The ensemble literature optimizes composition
for capability; our frame asks the orthogonal question --- whether the
ensemble's verification blind spots differ from the generator's (§4.1).
...
specification-gaming concept. \emph{Consort} (Hartman 2026) is the
cohort's implementation-level realization of the decoupling pattern: a
spec-first framework in which ``the agent that writes the code is never
the one that judges it'' --- the approved test list is hashed and locked
at a gate, in-cycle attempts to delete or weaken a test are caught and
flagged, and success is recorded only when a real test runner passes
against a real branch, not when the agent asserts it. Consort enforces
writer/judge separation as mechanism; it does not theorize the axis ---
specification gaming as an orthogonal failure class, the salient/latent
split, and the platform-boundary derivation (§4.4) are absent from its
framing, which is engineering-first. We regard it as evidence the
discipline is partially practiced before being theorized, in the same
sense as LoopX's evaluation subsystem below. \emph{Loki Mode}
...
that surface), none of the five makes the irreducibility argument (§3.2)
...
\textbf{A production-scale external instance: the Hermes self-refactor.}
The largest published worktree-parallel agent refactor --- 1,393
subagents over roughly nineteen active hours, reducing non-test Python
by 34.4\% (Teknium 2026) --- is, read through our frame, an external
corroboration of both §4.3's bifurcation and §6.5's
fix-introduced-defect class. Its automated interface-preservation checks
held (tool JSON schemas required to remain identical; CLI
\texttt{-\/-help} output compared byte for byte), yet two regression
classes the existing tests missed surfaced in human community review
rather than in the agents' gates: ``public names that workers had
removed because they had no callers inside the repository, even though
external plugins could import them'' --- optimizing the in-repo
caller-graph proxy while the true objective included external API
consumers (the §3.1 manifestation above) --- and ``an automated rewrite
of \texttt{suppress()} calls also changed exception handling at roughly
65 sites,'' a batch fix-introduced defect. The response was exactly the
salient-defense codification §4.3 predicts: ``a check that flags removed
public names and tests for review.'' The latent breaches were found by
the only oracle in the loop that was not the agent's own gates: human
reviewers --- consistent with the frame, though the report itself does
not theorize it.
...
\item
  Ma, Z., Huang, H., Zou, S., Wang, Y., Yang, S., Hu, Y., Wei, F., \&
  Chu, X. (2026). \emph{LongHorizon-Harness: Advancing Long-Horizon
  Agents for Real-World Tasks.} arXiv:2608.01964.
\item
  Hartman, K. (2026). \emph{Introducing Consort: A Spec-First Agent
  Framework for Enforced, Test-Driven Development on Live Database
  Branches.} arXiv:2609.09671.
\item
  Lodkaew, T., Ackermann, J., Nishimori, S., Charoenphakdee, N.,
  Sugiyama, M., \& Ishida, T. (2026). \emph{Do Coding Agents Deceive Us?
  Detecting and Preventing Cheating via Capped Evaluation with
  Randomized Tests.} arXiv:2606.07379.
\item
  Wataoka, K., Takahashi, T., \& Ri, R. (2024). \emph{Self-Preference
  Bias in LLM-as-a-Judge.} arXiv:2410.21819.
\item
  Panickssery, A., Bowman, S. R., \& Feng, S. (2024). \emph{LLM
  Evaluators Recognize and Favor Their Own Generations.}
  arXiv:2404.13076.
\item
  Lightman, H., Kosaraju, V., Burda, Y., Edwards, H., Baker, B., Lee,
  T., Leike, J., Schulman, J., Sutskever, I., \& Cobbe, K. (2023).
  \emph{Let's Verify Step by Step.} arXiv:2305.20050.
\item
  Deshpande, D., Kannappan, A., \& Qian, R. (2026). \emph{Benchmarking
  Reward Hack Detection in Code Environments via Contrastive Analysis.}
  arXiv:2601.20103.
\item
  Vallecillos-Ruiz, F., Hort, M., \& Moonen, L. (2025). \emph{Wisdom and
  Delusion of LLM Ensembles for Code Generation and Repair.}
  arXiv:2510.21513.
\item
  Wahi, V. (2026). \emph{LLM-as-a-Judge Is Not an Oracle: Why
  Self-Improving Agents Need Deterministic Guardrails.}
  arXiv:2609.02246.
\item
  Teknium. (2026). \emph{Refactoring Hermes with 1,393 agents.} Nous
  Research.
