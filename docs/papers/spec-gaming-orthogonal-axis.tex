...
\subsection{6.5 Retrospective evidence from production runs}\label{retrospective-evidence}
...
Findings are \emph{unadjudicated reviewer claims} --- counts are
claim-counts, not confirmed-defect counts; severity strata are reported
separately and honest-disclosure items are excluded from defect
denominators.

\textbf{Signature (i) --- leg divergence.} Among the 359
defect-severity findings whose producing leg is attributable from
surviving per-leg records, the same-source leg alone discovered 192
(53.5\%) and the heterogeneous leg alone 167 (46.5\%). Within this
attribution coverage (54\% of defect findings; the remainder sit in
units without per-leg records), each leg alone would have lost roughly
half of what the pair recovered. This is the retrospective analogue of
the divergence observation in Open Problem 8 --- not the known-gap test
itself: without ground truth on which findings are true defects,
divergence here measures discovery complementarity, not verdict
correctity. Both-report duplication cannot be measured at id level
(cross-leg identifiers never coincide); a kind-and-location pair
adjudication would move findings from single-leg buckets into the
both-bucket and cannot remove them, so the single-leg loss estimate is
an upper bound.
...
multi-round defect findings), spot-checked against record full text
with zero disagreement (5/5 positives, 3 negatives drawn / 2 effective
after one unverifiable lookup-miss; delegated-human tier, disclosed),
with a second independent seat re-adjudicating a 20\% subsample at
96.9\% strict-match agreement (zero genuinely disputed positives ---
an initial bare-id computation had manufactured 2 spurious disputes via
cross-unit id collision, corrected against the preserved subsample),
classifies 23 findings (3.4\% of all multi-round defect findings; 7.2\%
of the enriched candidate pool) as defects explicitly attributed by the
finding itself to an \emph{earlier fix} in the same run --- residues
of incomplete class-fixes, overshoots, and rewording regressions. The
pattern is pervasive across runs and engines (positives recorded in
five of the ten units the candidate pool spans) rather than an
artifact of any single integration. Every such finding was caught
within the loop, by a reviewer context that did not author the fix
under review. The causal reading --- ``caught only because the
reviewer was fresh'' --- is a counterfactual this corpus cannot carry
and we do not claim it.

\textbf{Limitations.} No control baseline exists (nothing here
compares the loop against single-pass review); environment provenance
is recoverable for a minority of units, so results are not stratified
by model era; the corpus is a survivorship-biased snapshot (degraded
and unrecorded runs are underrepresented); the coding pass is
single-seat with a consistency re-run reported alongside. A per-leg
attribution pass (multi-method: leg-file membership, cross-leg pair
adjudication, identifier-naming convention, peer-overlap) resolved 9
of the 12 originally unattributed fix-introduced positives, yielding a
same-source / heterogeneous split of 17 / 3 --- a skew \emph{toward}
the same-family reviewer, not away. The heterogeneous leg's unique
contribution to this corpus is discovery complementarity (the 44.5\%
of attributable findings only it found), not fix-residue detection
--- a genuinely unexpected asymmetry worth noting as a limitation on
any strong reading of the heterogeneous oracle's role. These bound
the section's claim to: in this production corpus, the two oracle
legs' discoveries are near-symmetric complements, and fix-introduced
defects are a recurring, catchable class --- consistent with, not
proof of, the two-axis frame's predictions.
...
\item
  Krakovna, V., et al. (2020). \emph{Specification gaming: the flip side of
...
\item
  ...
  arXiv:2507.05619. (ACL 2026 Findings version: Akter et al.)
\item
