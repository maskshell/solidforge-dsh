...
run_suite parallel-development \
  ...
  arm_report_gates.py arm_revert.py arm_neutral_config.py hetero_review_wiring.py \
  plan_queue_detect.py plan_queue_loop_state_wiring.py scope_check.py smoke_gates.py \
  drift_check.py lint_self.py

run_suite blueprint-crafting \
  plan_model_schema.py run_record_schema.py run_record.py produce_goldens.py \
  constraints_check_goldens.py research_constraints_goldens.py freeze_goldens.py \
  normalizer_goldens.py end_to_end.py round_trip.py plan_reviewer_precision.py \
  trigger_check.py disconnect_check.py lint_self.py
...
run_suite primary-source-verification \
  ...
  disconnect_check.py plugin_layout.py dogfood.py lint_self.py
